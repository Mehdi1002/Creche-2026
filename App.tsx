...
const App: React.FC = () => {
  ...
  // Initialisation instantanée depuis le stockage local (0 milliseconde d'attente)
  ...
  const [isLoading, setIsLoading] = useState<boolean>(() => children.length === 0 && isSupabaseConfigured);
  ...
  const getPaymentStatus = useCallback((childId: string, year: number, month: number) => {
    ...
    if (Number(p.amountPaid) >= MONTHLY_FEE) return { status: 'Payé', color: 'text-emerald-700', bg: 'bg-emerald-50', border: 'border-emerald-200' };
  ...
  // Mise à jour optimiste immédiate (0ms lag)
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        // Mise à jour locale immédiate (0ms)
                  ...
                  <tbody className="divide-y divide-slate-200">
                    {filteredChildren.length === 0 ? (
                      ...
                      filteredChildren.map(child => {
                        ...
                        const reste = p ? Math.max(0, MONTHLY_FEE - Number(p.amountPaid)) : MONTHLY_FEE;
